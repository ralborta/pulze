...
import { UtensilsCrossed, Plus, Search, Edit, Trash2, Tag, X, Save, Loader2 } from 'lucide-react'
import { api, type Content } from '@/lib/api'

const TYPES = ['tip', 'guia', 'articulo']
...
export default function NutricionPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['contents', 'Nutrición', typeFilter],
    queryFn: () =>
      api.contents.list({
        category: 'Nutrición',
  ...
  return (
    <>
      ...
      <div className="glass rounded-2xl sm:rounded-3xl p-4 sm:p-6 mb-4 sm:mb-6">
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Buscar contenidos de nutrición..."
              value={search}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <QuickStat
          label="Artículos"
          value={String(contents.filter((c) => c.type === 'articulo').length)}
          color="orange"
        />
      </div>
  ...
  return (
    <div className="glass rounded-2xl p-6 border border-white/10 hover:border-cyan-500/30 transition group">
      <div className="flex items-start justify-between mb-4">
        <span className="text-xs px-2 py-1 rounded-full font-medium border bg-orange-500/10 text-orange-400 border-orange-500/30">
          {content.type}
        </span>
        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
          <button onClick={onEdit} className="p-2 bg-white/5 hover:bg-white/10 rounded-lg transition">
            <Edit className="w-4 h-4 text-cyan-400" />
          </button>
          <button onClick={onDelete} className="p-2 bg-white/5 hover:bg-red-500/20 rounded-lg transition">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Guía de macros por objetivo"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500/50"
              placeholder="macros, proteína, hidratación"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              placeholder="Guías, recomendaciones, macros. La IA usará esto como base en consultas de nutrición."
            />
          </div>
